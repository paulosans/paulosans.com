...
export default function CasesPage() {
  ...
  const goTo = useCallback(
    (index: number, dir: 1 | -1) => {
      if (index < 0 || index >= CASES.length || scrollLock.current) return;
  ...
  return (
    <>
      ...
      {/* ══════════════════════════════════════ DESKTOP ══ */}
      <div className="cases-desktop hidden lg:block relative w-full h-screen overflow-hidden bg-white dark:bg-[#111318] transition-colors duration-300 select-none">

        {/* ── Nav ─────────────────────────────────────────────── */}
        <nav className="cases-nav absolute top-0 right-0 flex items-center gap-8 pt-14 pr-12 z-20">
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            {/* Brand name managed by LayoutProvider (layout-hero-text-portal) */}

            {/* Description */}
            <p
              className="cases-description absolute pointer-events-none z-20"
              style={{
                fontFamily: "'Agrandir Narrow', sans-serif",
                fontSize: "22px",
                letterSpacing: "0.5px",
                color: "rgba(36,42,47,0.56)",
                left: current.descLeft,
                top: current.descTop,
            ...
            {/* Next case — peeking at the bottom, clickable */}
            {current.nextCase && (
              <button
      ...
      <div className="cases-mobile flex lg:hidden flex-col min-h-screen bg-white dark:bg-[#111318] transition-colors duration-300 overflow-hidden">
        ...
        <nav className="cases-mobile-nav flex items-center justify-end gap-4 pt-10 pr-4 sm:pr-8 pl-4 sm:pl-8 flex-shrink-0">
          <div className="flex items-center justify-center px-2 py-2 bg-[rgba(36,42,47,0.08)] rounded-[8px]">
            <span className="font-['Agrandir'] font-[800] text-[#242a2f] dark:text-[#f0ede8] text-base tracking-[0.72px] whitespace-nowrap">
              Cases
            </span>
          </div>
        ...
        <div className="relative flex-1 overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              {/* Brand name */}
              <p
                className="font-['Agrandir'] font-[800] text-[#242a2f] dark:text-[#f0ede8] text-center leading-none"
                style={{ fontSize: "clamp(40px, 12vw, 80px)" }}
              >
                {current.name}
              </p>
              ...
              {/* Description */}
              <p
                className="font-['Agrandir_Narrow'] text-[rgba(36,42,47,0.56)] dark:text-[rgba(240,237,232,0.56)] text-[18px] tracking-[0.5px] text-center mt-4"
                style={{ lineHeight: "28px", whiteSpace: "pre-line" }}
              >
                {current.description}
              </p>
            </motion.div>
